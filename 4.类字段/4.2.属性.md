...
>
> 原因是我们想要在给字段赋值时，setter 可以作为 右手表达式（right-side expression) 使用。从而使用诸如 `x = y = 1` 的链式的语法结构，这种链式结构会被解析为 `x = (y = 1)`。而为了能够把 `y = 1` 的结果赋值给 `x`，表达式必须能够返回一个值，而如果说 `y` 有一个 setter 并且会返回 `Void` ，那么这种链式的结构就没有意义了。
>
